...
generate_update_call!(update_bucket_canister_wasm);
...
pub mod happy_path {
    ...
    use index_canister::user::UserRecord;
    use types::CanisterId;
    use utils::hasher::hash_bytes;

    pub fn add_or_update_users(env: &mut StateMachine, sender: Principal, canister_id: CanisterId, users: Vec<UserConfig>) {
        let response =
            super::add_or_update_users(env, sender, canister_id, &index_canister::add_or_update_users::Args { users });

        assert!(matches!(response, index_canister::add_or_update_users::Response::Success));
    ...
    pub fn allocated_bucket(
        ...
        file: &[u8],
    ) -> index_canister::allocated_bucket_v2::SuccessResult {
    ...
    pub fn user(env: &StateMachine, sender: Principal, canister_id: CanisterId) -> UserRecord {
        let response = super::user(env, sender, canister_id, &index_canister::user::Args {});

        if let index_canister::user::Response::Success(result) = response {
            result
        } else {
