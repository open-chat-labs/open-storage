...
use std::borrow::Cow;
use types::{CanisterId, FileAdded, FileId, FileRemoved, Hash, TimestampMillis, UserId};
...
impl Files {
    ...
    pub fn iter_user_files_from_oldest(&self, user_id: UserId) -> impl Iterator<Item = UserFile> + '_ {
...
mod tests {
    ...
    fn iter_user_files_from_oldest_returns_files_for_correct_user() {
        ...
        for u in 0..5 {
            let user_id = UserId::from_slice(&[u]);
            let start = u * 10;
        ...
        let created_dates: Vec<_> = files
            .iter_user_files_from_oldest(UserId::from_slice(&[3]))
            .map(|f| f.created)
            .collect();
